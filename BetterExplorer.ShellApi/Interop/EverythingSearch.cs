...
public static class EverythingSearch
{
        ...
        ChannelWriter<ShellItem> writer)
    {
        ...
        Debug.WriteLine($"[EverythingSearch] count={count}  prefix={folderPrefix}");
        ...
        for (uint i = 0; i < count; i++)
        {
            ...
            // Scope: only emit results that live under folderPrefix.
            ...
            long sizeBytes = 0;
            if (!isDir) _getResultSize!(i, out sizeBytes);

            DateTime dateModified = DateTime.Now;
            if (_getResultDateModified!(i, out long ft) && ft > 0)
            {
                try { dateModified = DateTime.FromFileTimeUtc(ft).ToLocalTime(); }
                catch { /* leave as Now */ }
            }
    ...
    private static bool TryLoad()
    {
        ...
        IntPtr hModule;
        try   { hModule = NativeLibrary.Load(dllPath); }
        catch (Exception ex) { Debug.WriteLine($"[EverythingSearch] NativeLibrary.Load failed: {ex.Message}"); return false; }
    ...
    private static string? FindEverythingDll()
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(pf))   dirs.Add(Path.Combine(pf,   "Everything"));
            if (!string.IsNullOrEmpty(pf86)) dirs.Add(Path.Combine(pf86, "Everything"));
        }
        catch { }
    ...
    private static string FormatSize(long bytes)
    {
        ...
        if (bytes < 1_024 * 1_024)           return $"{bytes / 1_024.0:F1} KB";
        if (bytes < 1_024L * 1_024 * 1_024)  return $"{bytes / (1_024.0 * 1_024):F1} MB";
